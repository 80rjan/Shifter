...
import {indexToDifficultyMapper, queryToDurationMapper, queryToPriceMapper} from "../utils/mapper.ts";
...
function CoursesGrid({courses, setFilters, filters}: {
    ...
    return (
        <section className="flex flex-col gap-8 py-6 pb-12 w-full col-span-5">
            ...
            <div className="flex justify-between items-center w-full px-12">
                                        ...
                                        key === "price" ?
                                            ...
                                            key === "duration" ?
                                                queryToDurationMapper(element)
                                                    ...
                                                    .replace(/\b\w/g, c => c.toUpperCase()) :
                                                key === "difficulty" ?
                                                    indexToDifficultyMapper(Number(element), t("difficultyOptions", { returnObjects: true }) as string[])
                                                        .toLowerCase()
                                                        .replace(/\b\w/g, c => c.toUpperCase()) :
                                                    element
                                                        .replace(/_/g, " ")
                ...
                <form
                    ...
                    <button
                        ...
                        ml-4 px-8 bg-shifter text-white rounded-sm border-2 border-white/40 cursor-pointer whitespace-nowrap
                    ">
                        {t("discover")}
                    </button>
                </form>
            ...
            <div className="relative grid grid-cols-3 gap-x-4 gap-y-4 w-full h-fit px-12">
                {
                    courses && courses?.length > 0 ?
                        ...
                        <span
                            className="text-3xl font-normal text-black/60 pl-20 py-8 text-left col-span-full">{t("error")}</span>
                }
            </div>
        </section>
    )
