...
public class CourseDtoBuilder {
    ...
    private <T extends CourseDtoPreview> T enrichCourseDto(Course course, T dto, Language language) {
        ...
        CourseVersion courseVersion = courseVersionRepository.findByActiveTrueAndCourse_Id(course.getId());
        int contentCount = courseVersion.getCourseContents() != null ? courseVersion.getCourseContents().size() : 0;
        int lectureCount = courseVersion.getCourseContents() != null
                ? courseVersion.getCourseContents().stream()
    ...
    public CourseDtoFull getCourseDtoFull(Course course) {
        ...
        if (dto.getCourseContents() == null || dto.getCourseContents().isEmpty()) {
            dto.setCourseContents(
                    courseContentMapper.toDtoFull(courseVersion.getCourseContents(), Language.EN)
            );
        }
