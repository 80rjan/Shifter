...
import com.shifterwebapp.shifter.course.dto.CourseDtoBuilder;
...
public class AdminCourseService implements ImplAdminCourseService{
    ...
    @Override
    public Course createCourse(CourseDtoFull courseDtoFull) {
        ...
        List<CourseContent> contentList = courseDtoFull.getCourseContents().stream()
                .map(content -> courseContentService.buildCourseContent(content, course, language))
                .toList();

        course.setCourseContents(contentList);
    ...
    private Course buildCourse(CourseDtoFull courseDtoFull, Language language) {
        ...
        course.setCourseTranslates(List.of(courseTranslate));
        return course;
    }
...
//        validate.validateCourseTranslation(courseDtoTranslate.getId(), courseDtoTranslate.getLanguage());
        ...
        translateContents(course, courseDtoTranslate.getCourseContents(), courseDtoTranslate.getLanguage());
    ...
    private void addCourseTranslation(Course course, CourseDtoTranslate dto) {
        ...
        course.getCourseTranslates().add(translation);
    }

    private void translateContents(Course course, List<CourseContentDtoTranslate> contentDtos, Language language) {
    ...
    private void addContentTranslation(CourseContent content, CourseContentDtoTranslate contentDto, Language language) {
        ...
        content.getCourseContentTranslates().add(contentTranslate);
    ...
    private void translateLectures(CourseContent content, List<CourseLectureDtoTranslate> lectureDtos, Language language) {
        for (CourseLecture lecture : content.getCourseLectures()) {
            ...
            lecture.getCourseLectureTranslates().add(lectureTranslate);
        }
    }
